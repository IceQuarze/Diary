<?php
    ...
    include 'RESTful.php';
    include 'config.php';
    $db = new DiaryDatabase('localhost', 'diary', 'diary');
    ...
    switch ($data['op']) {
        ...
        case 'getIndex':
            ...
            while ($row = mysqli_fetch_array($result)) {
                $user_id = (object)array('userId' => $row['user_id']);
		file_put_contents('/var/www/html/php_var.log',var_export($user_id,TRUE));
                $info = json_decode(RestUtils:: http_request( $info_url,$user_id), TRUE);
                $list[] = (object)array
                (
